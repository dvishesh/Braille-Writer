 ...
 *****************************************************************************/
 ...
 // REGISTER READ/WRITE FUNCTIONS
     ...
     /*@ Functions sets CTRL_REG1 bits - DATA RATE& ACTIVE/STANDBY MODE ACCESSIBLE FROM HERE
       Note: Might have to be configured to accept I2Cx & ACC_I2C_ADDRx as parameters
     */
     ...
     static void setControlReg1(uint8_t registervalue)
     {
         ...
         buf[0] = CTRL_REG1 ;
         buf[1] = registervalue;
         Chip_I2C_MasterSend(I2C0, ACC_I2C_ADDRESS, buf, 2);
     }
     ...
     */
     ...
     /*@ Functions sets CTRL_REG2 bits - RESET/ SLEEP MODE OR ACTIVE MODE POWER SCHEME / AUTO-SLEEP ENABLE can be accessed from here
       Note: Might have to be configured to accept ACC_I2C_ADDRx as a parameter
     */
      ...
      static uint8_t getModeControlReg1(void)
          	 {
          	     ...
          	     Chip_I2C_MasterCmdRead(I2C0, ACC_I2C_ADDRESS, CTRL_REG1, buf, 1);
          	     registervalue = buf[0];
...
*****************************************************************************/
     ...
     void ReadPulseSource(void)
     {
    	 ...
    	 Chip_I2C_MasterCmdRead(I2C0, ACC_I2C_ADDRESS, PULSE_SRC, buf, 1);
     }
     ...
     uint8_t GetINT_SRC_Status(void)
     {
    	 ...
    	 Chip_I2C_MasterCmdRead(I2C0, ACC_I2C_ADDRESS, INT_SOURCE, buf, 1);
    	 return buf[0];
     ...
     uint8_t Get_STATUS(void)
     {
    	 ...
    	 Chip_I2C_MasterCmdRead(I2C0, ACC_I2C_ADDRESS, STATUS, buf, 1);
         return buf[0];
     }
     ...
     uint8_t Get_WHOAMI(void)
        {
       	...
       	buf[0] = 0;
       	 ...
       	 Chip_I2C_MasterCmdRead(I2C0, ACC_I2C_ADDRESS, WHO_AM_I, buf, 1);
            return buf[0];
        }
     ...
     uint8_t Get_ZPULSETHRESHOLD(void)
             {
            	...
            	buf[0] = 0;
            	 ...
            	 Chip_I2C_MasterCmdRead(I2C0, ACC_I2C_ADDRESS, PULSE_THSZ , buf, 1);
                 return buf[0];
             }
     ...
     uint8_t Get_ZPULSE_TMLIMIT(void)
             {
            	...
            	buf[0] = 0;
            	 ...
            	 Chip_I2C_MasterCmdRead(I2C0, ACC_I2C_ADDRESS, PULSE_TMLT , buf, 1);
                 return buf[0];
             }
     ...
     uint8_t Get_ZPULSE_TMLATENCY(void)
             {
            	...
            	buf[0] = 0;
            	 ...
            	 Chip_I2C_MasterCmdRead(I2C0, ACC_I2C_ADDRESS, PULSE_LTCY , buf, 1);
                 return buf[0];
             }
     ...
     void acc_read (int16_t *z)
     {
        ...
        Chip_I2C_MasterCmdRead(I2C0, ACC_I2C_ADDRESS, OUT_Z_MSB , zvalue_hi, 1);
        Chip_I2C_MasterCmdRead(I2C0, ACC_I2C_ADDRESS, OUT_Z_LSB , zvalue_low, 1);
